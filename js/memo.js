(function () {
    const cards = document.getElementsByClassName('memo-card');
    const stars = document.getElementsByClassName('fa-star');
    const restart = document.getElementById('memo-restart');
    const again = document.getElementById('memo-again');
    const grats = document.getElementById('memo-grats');
    let moves = document.getElementById('memo-moves');
    let scores = document.getElementById('memo-stars-score');
    let clear = document.getElementById('memo-clear-time');
    let score = 3;
    ...
    function flip() { //flip card and start match() function
        ...
        if (temp.length < 2 && !this.classList.contains('memo-show')) {
            this.classList.add('memo-show');
        ...
        if (temp.length != 2) return; //do nothing if one card selected
            ...
            setTimeout(function () { //time for the animation ;)
                ...
                for (let card of cards) {
                    if (!card.classList.contains('memo-lock')) //don't remove 'show' class from matched cards
                        card.classList.remove('memo-show'); //unflip not matched cards
                }
            }, 1200);
            move();
            star();
            ...
            setTimeout(function () {
                for (let card of cards) {
                    if (card.classList.contains('memo-show')) { //lock matched cards with 'lock' class
                        card.classList.add('memo-lock');
    ...
    restart.addEventListener('click', memoReset); //restart button event listener
    again.addEventListener('click', function () { //modal 'play again' button event listener
        ...
        } else memoReset(); //if time less than 4 minutes just reset
    });
    ...
    function time() { //time count
        let inter = setInterval(function () {
            ...
            star();
            locked = document.getElementsByClassName('memo-lock'); //locked (matched) cards
            if (minutes === 4 && seconds === 0) { //changed modal (yoda's wisdom)
                ...
                grats.classList.remove('memo-hidden');
    ...
    function memoReset() { //function for reset game (all variables, time, move counter, ets)
        timerOn = false;
        for (let card of cards) { //prevent card animation and click on them
            card.classList.add('memo-animation-off');
            card.classList.remove('memo-show');
            card.classList.remove('memo-lock');
        ...
        setTimeout(function () { //timeout for end of function
            for (let card of cards) {
                card.classList.remove('memo-animation-off');
            }
            timer.textContent = '00:00';
            init();
        }, 700);
        if (!grats.classList.contains('memo-hidden')) //if NOT
            grats.classList.add('memo-hidden');
    }
